...
use crate::{get_type, to_toml_v1, toml_dt_v1, toml_rs::TOMLEncodeError};
...
fn to_toml<'py>(
    ...
    obj: &Bound<'py, PyAny>,
    inline_tables: Option<&FxHashSet<String>>,
    toml_path: &mut SmallVec<String, 32>,
) -> PyResult<Item> {
    ...
    if let Ok(int) = obj.cast::<PyInt>() {
        return to_toml_v1!(BigNum, int.str()?.to_str()?);
    }
    if let Ok(float) = obj.cast::<PyFloat>() {
        return to_toml_v1!(BigNum, float.str()?.to_str()?);
    }
    ...
    if let Ok(dict) = obj.cast::<PyDict>() {
        ...
        return if inline {
            ...
            for (k, v) in dict.iter() {
                ...
                let item = to_toml(py, &v, inline_tables, toml_path)?;
                toml_path.pop();
    ...
    if let Ok(list) = obj.cast::<PyList>() {
        ...
        for item in list.iter() {
            let items = to_toml(py, &item, inline_tables, toml_path)?;
            match items {
                Item::Value(value) => {
                    array.push(value);
                }
