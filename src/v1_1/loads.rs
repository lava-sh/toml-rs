...
use crate::{create_py_datetime, error::DecodeError, parse_int};
...
impl<'py, 'i> RawReceiver<'py, 'i, '_> {
    ...
    fn build_datetime(&mut self, decoded: &str, span: Span) -> PyResult<Option<Bound<'py, PyAny>>> {
        let py = self.py;
        let datetime = match decoded.parse::<toml::value::Datetime>() {
        ...
        let value = match (datetime.date, datetime.time, datetime.offset) {
            (Some(date), Some(time), Some(offset)) => {
                ...
                create_py_datetime!(py, date, time, Some(&tzinfo))?.into_any()
            }
            (Some(date), Some(time), None) => create_py_datetime!(py, date, time, None)?.into_any(),
            (Some(date), None, None) => {
...
pub fn load<'py>(
    ...
    let mut errors = Option::<ParseError>::None;
    {
        ...
        toml_parser::parser::parse_document(&tokens, &mut validated, &mut errors);
    }
